...
def read_magicNumbers(srcDir):
    magicNumber_list = []
    for dirpath, dirnames, filenames in os.walk(srcDir):
        for filename in filenames:
            if filename.endswith(".c.pp"):
                #print('Processing ' + os.path.join(dirpath, filename))
                code_dict = get_func_code(os.path.join(dirpath, filename))
                for key, code in code_dict.items():
                    #print(len(code))
                    magicNumbers = re.findall('\d+', code)
                    #print(magicNumbers)
                    for magicNumber in magicNumbers:
                        if int(magicNumber) not in magicNumber_list:
                            magicNumber_list.append(int(magicNumber))
    #print(magicNumber_list)
    #print(len(magicNumber_list))
    #print("------------------------------")
    return magicNumber_list

def read_enumNumbers(srcDir):
    enumNumber_list = []
    enumNumber_dict = {}
    for dirpath, dirnames, filenames in os.walk(srcDir):
        for filename in filenames:
            if filename.endswith(".c.pp"):
                ...
                with open(os.path.join(dirpath, filename), 'r') as f:
                    lines = f.readlines()
                #print(len(code))
                enumNumbers = re.findall('enum\s{[\s\S]*?}', '\n'.join(lines))
                #print(re.findall('enum\s{(.*?\n*)*}', '\n'.join(lines)))
                #print(re.findall('enum\s{[\s\S]*?}', lines))
                #print(enumNumbers)
                for enumNumber in enumNumbers:
                    lastMagicNumber = 0
                    tokens = re.sub('[\s]*?', ' ', enumNumber).split()
                    start_index = tokens.index('{') + 1
                    end_index = tokens.index('}')
                    temp = ''.join(tokens[start_index : end_index])
                    for token in temp.split(','):
                        if len(token) > 0:
                            if '=' in token:
                                magicNumbers = re.findall('\d+', token.split('=')[-1])
                                for magicNumber in magicNumbers:
                                    lastMagicNumber = int(magicNumber)
                                    enumNumber_dict[token.split('=')[0]] = lastMagicNumber
                            else:
                                enumNumber_dict[token.split('=')[0]] = lastMagicNumber
                                lastMagicNumber += 1
                code_dict = get_func_code(os.path.join(dirpath, filename))
                for key, code in code_dict.items():
                    for enum_name, enum_value in enumNumber_dict.items():
                        if enum_name in code:
                            if enum_value not in enumNumber_list:
                                enumNumber_list.append(enum_value)


    #print(len(enumNumber_list))
    #print(enumNumber_list)
    #print("------------------------------")
    return enumNumber_list
...
def coverage_increased(covFile, i):
    ...
    try:
        ...
        if result == True:
            lines = []
            with open(covResultFile, 'r') as f:
                lines = f.readlines()
            for line in lines[-3:]:
                tokens = line.split(',')
                if len(tokens) > 0:
...
def cross_mutate(seedInput_list, magicNumber_list, priority, epoch_count):
    ...
    tries = (crossover_mutation_count) * 10
...
def run_epoch(exeFile, covFile, crashFile, hangFile, callGraph, testId_list, magicNumber_list, highFunc_list, highInput_list, lowInput_list, priority, result_dict, allInputTuple_list, allInput_list, allUniqueInput_list, allUniqueValidInput_list):
    ...
    while True:
        ...
        for seedInput_index, seedInput in enumerate(seedInput_list):
            ...
            if len(mutatedInput_list) > 0:
                mutatedInput_list_part_length = 1000
                for mutatedInput_list_part_index, mutatedInput_list_part in enumerate(list(chunks(mutatedInput_list, mutatedInput_list_part_length))):
                    ...
                    try:
                        ...
                        subprocess.run([shutil.which('covfn'), '-f', epochCovFile, '-c', '--no-banner', '--output', epochCovResultFile])
                        if filecmp.cmp(epochCovResultFile, covResultBackupFile):
                            # coverage did not change
                            epochCoverageIncreased = False
                        else:
                            lines = []
                            with open(epochCovResultFile, 'r') as f:
                                lines = f.readlines()
                            for line in lines[-3:]:
                                tokens = line.split(',')
                                if len(tokens) > 0:
                    ...
                    for mutatedInput_index, input_list in enumerate(mutatedInput_list_part):
                        ...
                        if epochCoverageIncreased == True and (checkepochCoverageIncreased == False or epochCoverage > mcdc_coverage):
                            ...
                            old_mcdc_coverage = mcdc_coverage
                            ...
                            if result_coverage == True:
                                ...
                                if old_mcdc_coverage == mcdc_coverage:
                                    print(input_list)
        ...
        while True:
            while True:
                ...
                if len(highInput_list) + len(lowInput_list) == 0:
                    break
            break
            # this epoch runs mutations takes a bit longer.
            #run_epoch(exeFile, covFile, crashFile, hangFile, callGraph, testId_list, magicNumber_list, highFunc_list, highInput_list, lowInput_list, 'UNIQUE', result_dict, allInputTuple_list, allInput_list, allUniqueInput_list, allUniqueValidInput_list)
            #if len(highInput_list) + len(lowInput_list) == 0:
            #    run_epoch(exeFile, covFile, crashFile, hangFile, callGraph, testId_list, magicNumber_list, highFunc_list, highInput_list, lowInput_list, 'ALL', result_dict, allInputTuple_list, allInput_list, allUniqueInput_list, allUniqueValidInput_list)
...
def run_tests(exeFile, covFile, crashFile, hangFile, callGraphFile, funcweight_dict, highInput_list, lowInput_list, allInput_list):
    ...
    testId_list = []
    magicNumber_list = []
    enumNumber_list = read_enumNumbers("./../proposed/out")
    magicNumber_list = read_magicNumbers("./../proposed/out")
    for enumNumber in enumNumber_list:
        if enumNumber not in magicNumber_list:
            magicNumber_list.append(enumNumber)
