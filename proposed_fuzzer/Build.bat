...
@echo ====================================
%PYTHON_PATH%\python Executor.py -i%TESTDIR%\inputs -e%OUTDIR%\test.exe -c%COVFILE% -w.\..\proposed\metrics\FunctionWeight.txt -g.\..\proposed\metrics\CallGraph.txt --high=%OUTDIR%\high.txt --low=%OUTDIR%\low.txt --all=%OUTDIR%\all.txt

@echo ====================================
@echo Fuzz Testing Instrumented Binaries
@echo ====================================
for /l %%N in (1 1 5) do %PYTHON_PATH%\python Fuzz.py -e%OUTDIR%\test.exe -c%COVFILE% -w.\..\proposed\metrics\FunctionWeight.txt -g.\..\proposed\metrics\CallGraph.txt --high=%OUTDIR%\high.txt --low=%OUTDIR%\low.txt --all=%OUTDIR%\all.txt --valid=%OUTDIR%\valid.txt --hang=%OUTDIR%\hang.txt --crash=%OUTDIR%\crash.txt
