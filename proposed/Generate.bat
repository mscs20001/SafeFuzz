...
cd %INFRA_ROOT_DIR%/modules/%1
IF "%~2" == "" GOTO RunTests
@rem for %%a in ("unit_test/module_emb/src\*") do if /i not "%%~nxa"=="%2" del "%%a"
RunTests:
if exist out rmdir out /q /s
@echo make clean unit_test OFFLINE_MODE=y PARALLEL_BUILD=
make clean unit_test OFFLINE_MODE=y PARALLEL_BUILD=
cd %CWD%
